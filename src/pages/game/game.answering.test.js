...
test('checking adding pictures', () => {   
    ...
    expect(answering.changeImage(sampleAnswer[0])).toBe(pictureTag.complete && image.naturalHeight !== 0);
});
...
test('checking if at least one element has diffrent place', () => {
    ...
    expect(shuffleArray(arr)).toBe(
        arr.indexOf("a") !== 0 || arr.indexOf("b") !== 2 || arr.indexOf("c") !== 3 || arr.indexOf("d") !== 3
    );
})
