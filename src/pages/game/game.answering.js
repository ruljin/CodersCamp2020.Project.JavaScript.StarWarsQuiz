...
function showAnswers(options) {
  let questionsTags =
    '<div class="selector answers__item"><p>' +
    options[0] +
    '</p></div>' +
    '<div class="selector answers__item"><p>' +
    options[1] +
    ...
    options[2] +
    '</p></div>' +
    '<div class="selector answers__item correct"><p>' +
    options[3] +
    '</p></div>';
...
function selectAnswer(e) {
  ...
  if (selectedButton.innerText === correctAnswer['fields']['name']) {
    ...
    selectedButton.setAttribute(
      'class',
      'selector answers__item answers__item--blue'
    );
  } else {
    wrongAnswers++;
    selectedButton.setAttribute(
      'class',
      'selector answers__item answers__item--red'
    );
    for (var x = 0; x < answersButtons.length; x++) {
      if (answersButtons[x].innerText === correctAnswer['fields']['name']) {
        answersButtons[x].setAttribute(
          'class',
          'selector answers__item answers__item--blue'
        );
      }
